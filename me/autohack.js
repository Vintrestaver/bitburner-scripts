...
export async function main(ns) {
    ...
    ns.write(files[0], 'grow(args[0])', 'w'); ns.write(files[1], 'weaken(args[0])', 'w'); ns.write(files[2], 'hack(args[0])', 'w');
    ...
    function str(s) { if (s.length > 12) { return s.substring(0, 12) + '...' } else { return s } }
    function info(t, s) {
    ...
    function log() {
        ...
        for (let t of tmp) {
            ns.print(`║ ${act[t[1]]} ║ ${str(t[1])}` +
                `${ns.formatNumber(info('MM', t[1]), 1)}[${ns.formatPercent(info('MA', t[1]) / info('MM', t[1]), 1)}] ║`.padStart(36 - str(t[1]).length))
        }
        ns.print('╠═══╩════════════════════════════════════╝')
        ...
        for (let server of ns.scan(current)) {
            if ((ns.getPurchasedServers().includes(server) || info('NPR', server) <= exes.length) && host != server) {
                if (!ns.getPurchasedServers().includes(server)) {
                    for (let hack of exes) { ns[hack](server) }; ns.nuke(server)
                }
        ...
        for (let host of hosts) {
            ...
            if (info('MA', target) < info('MM', target) * .80) { hType = 0 }
                ...
                if (fRam() / info('MR', host[1]) > .13 && fRam() > 4) {
                    tmp = Math.floor(fRam() / 1.75); if (tmp > 0) {
                        ns.exec(files[1], host[1], tmp, target)
                    }
                }
            } else {
                hType = 2; for (let h of hosts) {
                    if (ns.isRunning(files[2], h[1], target) && h[1] != host[1]) { hType = 0; break }
                }
                if (hType == 2 && !ns.scriptRunning(files[2], host[1])) {
                    if (fRam() < 2) { ns.killall(host[1]) }
                    tmp = [1, Math.floor(fRam() / 1.7)];
                    while (ns.hackAnalyze(target) * tmp[0] < .7 && tmp[0] < tmp[1]) { tmp[0]++ }
            ...
            if (!loop) {
                if (hType == 0) { act[target] = 'G' };
                if (hType == 1) { act[target] = 'W' };
                if (hType == 2) { act[target] = 'H' };
            }
            tarIndex++;
        }
    }
